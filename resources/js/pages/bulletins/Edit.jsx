...
const BulletinsEdit = ({ bulletin }) => {
  // Prepare translation data
  const translationData = {
    fr: {
      title: bulletin.translations?.find(t => t.locale === 'fr')?.title || '',
      message: bulletin.translations?.find(t => t.locale === 'fr')?.message || ''
    },
    sr: {
      title: bulletin.translations?.find(t => t.locale === 'sr')?.title || '',
      message: bulletin.translations?.find(t => t.locale === 'sr')?.message || ''
    }
  }
  ...
  const { data, setData, put, processing, errors } = useForm({
    ...
    type: bulletin.type || 'general',
    expires_at: bulletin.expires_at ? new Date(bulletin.expires_at) : null,
    ...
    <Layout>
      <Container size="md" py="xl">
        <Paper p="xl" withBorder style={{ background: 'linear-gradient(135deg, rgba(26, 31, 46, 0.8) 0%, rgba(15, 20, 25, 0.9) 100%)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255, 215, 0, 0.2)', borderRadius: '16px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.4)' }}>
          ...
          <Title order={2} c="#FFD700" mb="md" style={{ fontFamily: 'serif' }}>Edit Bulletin</Title>
          <Text c="#b8b8b8" mb="xl">Update bulletin information</Text>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Select label="Status" value={data.status} onChange={(value) => setData('status', value)} error={errors.status} required data={[ { value: 'active', label: 'Active' }, { value: 'expired', label: 'Expired' }, { value: 'archived', label: 'Archived' } ]} mb="md" styles={{ label: { color: '#FFD700', fontWeight: 500, marginBottom: 8 }, input: { backgroundColor: 'rgba(26, 31, 46, 0.6)', border: '1px solid rgba(255, 215, 0, 0.2)', color: '#fff' } }} />
            ...
            <Group justify="flex-end">
              ...
              <Button type="submit" loading={processing} leftSection={<Save size={18} />} style={{ background: 'linear-gradient(135deg, #FFD700 0%, #FFA500 100%)', color: '#1a1f2e', fontWeight: 600 }}>Update Bulletin</Button>
            </Group>
          </form>
        </Paper>
      </Container>
